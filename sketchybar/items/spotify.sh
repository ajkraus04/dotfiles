#!/bin/bash

SPOTIFY_EVENT="com.spotify.client.PlaybackStateChanged"
sketchybar --add event spotify_change $SPOTIFY_EVENT \
  --add item spotify.anchor q \
  --set spotify.anchor script="$PLUGIN_DIR/spotify.sh" \
                       click_script="sketchybar --set spotify.anchor popup.drawing=toggle" \
                       popup.horizontal=on \
                       popup.align=center \
                       popup.height=150 \
                       icon=$SPOTIFY \
                       icon.font="$ICON_FONT:Regular:22.0" \
                       icon.color=$PINK \
                       icon.padding_right=6 \
                       label.font="$FONT:Regular:13.0" \
                       label.max_chars=30 \
                       drawing=on \
                       updates=on \
                       update_freq=5 \
  --subscribe spotify.anchor spotify_change mouse.exited.global \
  \
  --add item spotify.cover popup.spotify.anchor \
  --set spotify.cover script="$PLUGIN_DIR/spotify.sh" \
                      click_script="open -a 'Spotify'" \
                      label.drawing=off \
                      icon.drawing=off \
                      padding_left=12 \
                      padding_right=10 \
                      background.image.scale=0.2 \
                      background.image.drawing=on \
                      background.drawing=on \
                      background.image.corner_radius=9 \
  \
  --add item spotify.title popup.spotify.anchor \
  --set spotify.title icon.drawing=off \
                      padding_left=0 padding_right=0 width=0 \
                      label.font="$FONT:Bold:15.0" \
                      label.max_chars=20 \
                      y_offset=55 \
  \
  --add item spotify.artist popup.spotify.anchor \
  --set spotify.artist icon.drawing=off \
                       y_offset=30 \
                       padding_left=0 padding_right=0 width=0 \
                       label.max_chars=20 \
  \
  --add item spotify.album popup.spotify.anchor \
  --set spotify.album icon.drawing=off \
                      padding_left=0 padding_right=0 \
                      y_offset=15 width=0 \
                      label.max_chars=25 \
  \
  --add slider spotify.state popup.spotify.anchor \
  --set spotify.state icon.drawing=on \
                      icon.font="$FONT:Italic:10.0" \
                      icon.width=35 icon="00:00" \
                      label.drawing=on \
                      label.font="$FONT:Italic:10.0" \
                      label.width=35 label="00:00" \
                      padding_left=0 padding_right=0 \
                      y_offset=-15 width=0 \
                      slider.background.height=6 \
                      slider.background.corner_radius=1 \
                      slider.background.color=$GREY \
                      slider.highlight_color=$PINK \
                      slider.percentage=40 \
                      slider.width=115 \
                      script="$PLUGIN_DIR/spotify.sh" \
                      update_freq=1 updates=when_shown \
  --subscribe spotify.state mouse.clicked \
  \
  --add item spotify.shuffle popup.spotify.anchor \
  --set spotify.shuffle icon=$SPOTIFY_SHUFFLE \
                        icon.padding_left=5 icon.padding_right=5 \
                        icon.color=$BLACK icon.highlight_color=$GREY \
                        label.drawing=off y_offset=-45 \
                        script="$PLUGIN_DIR/spotify.sh" \
  --subscribe spotify.shuffle mouse.clicked \
  \
  --add item spotify.back popup.spotify.anchor \
  --set spotify.back icon=$SPOTIFY_BACK \
                     icon.padding_left=5 icon.padding_right=5 \
                     icon.color=$BLACK label.drawing=off y_offset=-45 \
                     script="$PLUGIN_DIR/spotify.sh" \
  --subscribe spotify.back mouse.clicked \
  \
  --add item spotify.play popup.spotify.anchor \
  --set spotify.play icon=$SPOTIFY_PLAY_PAUSE \
                     background.height=40 background.corner_radius=20 \
                     width=40 align=center \
                     background.color=$POPUP_BACKGROUND_COLOR \
                     background.border_color=$WHITE \
                     background.border_width=0 background.drawing=on \
                     icon.padding_left=4 icon.padding_right=5 \
                     updates=on label.drawing=off \
                     script="$PLUGIN_DIR/spotify.sh" y_offset=-45 \
  --subscribe spotify.play mouse.clicked spotify_change \
  \
  --add item spotify.next popup.spotify.anchor \
  --set spotify.next icon=$SPOTIFY_NEXT \
                     icon.padding_left=5 icon.padding_right=5 \
                     icon.color=$BLACK label.drawing=off y_offset=-45 \
                     script="$PLUGIN_DIR/spotify.sh" \
  --subscribe spotify.next mouse.clicked \
  \
  --add item spotify.repeat popup.spotify.anchor \
  --set spotify.repeat icon=$SPOTIFY_REPEAT \
                       icon.highlight_color=$GREY \
                       icon.padding_left=5 icon.padding_right=10 \
                       icon.color=$BLACK label.drawing=off y_offset=-45 \
                       script="$PLUGIN_DIR/spotify.sh" \
  --subscribe spotify.repeat mouse.clicked \
  \
  --add item spotify.spacer popup.spotify.anchor \
  --set spotify.spacer width=5 \
  \
  --add bracket spotify.controls spotify.shuffle spotify.back \
                                 spotify.play spotify.next spotify.repeat \
  --set spotify.controls background.color=$PINK \
                         background.corner_radius=11 \
                         background.drawing=on y_offset=-45
